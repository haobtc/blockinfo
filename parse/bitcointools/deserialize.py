...
def extract_public_key_and_address(bytes, version='\x00'):
  try:
    decoded = [ x for x in script_GetOp(bytes) ]
  except struct.error:
    return None, "(None)"
  ...
  match = [ opcodes.OP_PUSHDATA4, opcodes.OP_CHECKSIG ]
  if match_decoded(decoded, match):
    return decoded[0][1], public_key_to_bc_address(decoded[0][1], version=version)

  # Pay-by-Bitcoin-address TxOuts look like:
  # DUP HASH160 20 BYTES:... EQUALVERIFY CHECKSIG
  match = [ opcodes.OP_DUP, opcodes.OP_HASH160, opcodes.OP_PUSHDATA4, opcodes.OP_EQUALVERIFY, opcodes.OP_CHECKSIG ]
  if match_decoded(decoded, match):
    return decoded[2][1], hash_160_to_bc_address(decoded[2][1], version=version)
  ...
  for match in multisigs:
    if match_decoded(decoded, match):
      return decoded[i][1], "["+','.join([public_key_to_bc_address(decoded[i][1]) for i in range(1,len(decoded)-1)])+"]"

  # BIP16 TxOuts look like:
  # HASH160 20 BYTES:... EQUAL
  match = [ opcodes.OP_HASH160, 0x14, opcodes.OP_EQUAL ]
  if match_decoded(decoded, match):
    return decoded[1][1], hash_160_to_bc_address(decoded[1][1], version="\x05")
